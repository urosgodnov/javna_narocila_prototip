...
def get_dynamic_form_steps_refactored(session_state):
    ...
    if lot_mode == "none":
        ...
        # Check if we're loading a draft or in edit mode
        is_loading_draft = session_state.get("current_draft_id") is not None
        is_edit_mode = session_state.get("edit_mode", False)
        
        # If loading draft or edit mode, include ALL lot steps for proper navigation
        if is_loading_draft or is_edit_mode:
            # Add steps for ALL lots
            ...
            # Normal mode: Only add lot-specific steps if we have configured lots
