...
"""
DEPRECATED: This file is marked for removal.
Use ui/controllers/form_controller.py instead.
Removal date: 30 days from deployment (2024-02-29)
...
warnings.warn(
    "form_renderer module is deprecated and will be removed in 30 days. "
    "Use FormController from ui.controllers.form_controller instead. "
    "For gradual migration, use ui.form_renderer_compat.",
    DeprecationWarning,
    stacklevel=2
)
...
from utils.validations import get_validation_summary  # Still needed for render_validation_summary
...
def render_form(schema_properties, parent_key="", lot_context=None, field_manager=None, real_time_validator=None):
    ...
    DEPRECATED: This function is deprecated. Use FormController.render_form() instead.
    ...
    warnings.warn(
        "render_form() is deprecated and will be removed in 30 days. "
        "Use FormController.render_form() from ui.controllers.form_controller instead.",
        DeprecationWarning,
        stacklevel=2
    )
                    ...
                    with container:
                        ...
                        with col_remove:
                            if st.button("", key=f"widget_remove_{session_key}_{i}", help=f"Odstrani"):
                                # TODO(human): Implement comprehensive session state reindexing for array removal
                                # The current logic only removes from the main array but doesn't handle
                                # individual field session state keys that need reindexing
                                pass
